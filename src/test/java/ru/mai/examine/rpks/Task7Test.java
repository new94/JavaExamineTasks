...
@Slf4j
class Task7Test extends Infrastructure {
     ...
     * возвращаем в endpoint из найденных строк значение из поля test_value с максимальным id
     *
     ...
     */
    @Test
    void task7() throws Exception {
        this.mockMvc.perform(get("/task7?bootstrapServers=" + kafka.getBootstrapServers() + "&topic=test_topic_in"))
                .andDo(print())
                .andExpect(status().isOk())
