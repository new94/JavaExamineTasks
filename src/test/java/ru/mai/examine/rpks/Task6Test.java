...
@Slf4j
class Task6Test extends Infrastructure {
     ...
     * возвращаем в endpoint значение поля test_value_field из документа MongoDB в верхнем регистре и с обратным порядком букв
     * <p>
     * <p>
    ...
    void task6() throws Exception {
        this.mockMvc.perform(get("/task3?bootstrapServers=" + kafka.getBootstrapServers() + "&topic=test_topic_in"
                ...
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().string("EULAV_TSET"));
    }
